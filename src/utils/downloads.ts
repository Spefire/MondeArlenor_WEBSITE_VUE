...
const exportInsta = async(character: ArlenorCharacter, allSkills: ArlenorSkill[], allPowers: ArlenorPower[]): Promise<void> => {
  const doc = new jsPDF("p", "px", [1080, 2400]);
  const width = doc.internal.pageSize.getWidth();
  const height = doc.internal.pageSize.getHeight();
  let lineH = 10.55;

  const backgroundRecto = await require("../assets/files/FicheInsta.jpeg");
  doc.addImage(backgroundRecto, "JPEG", 0, 0, width, height);
  doc.setFontSize(10);
  ...
  doc.save("Arlenor_" + character.name + ".jpeg");
};
